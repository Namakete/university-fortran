program exercise_1_3
  ...
  (file=input_file, newunit=In)
      read (In, *) x
   close (In)

   (file=output_file, encoding=E_, newunit=Out)
      fmt = "(a, T7, '= ', f6.2)"
      write (Out, fmt) "x", x
   close (Out)
   ...
   = Items / [1, -2, 3, -4] ! ВЕКТОРИЗАЦИЯ. SSE, AVX
   ...
   (file=output_file, encoding=E_, newunit=Out, position='append')
