...
contains
    pure function Sort_class_list(Group) result (Max_Amount_Index)
        type(student), intent(in)   :: Group(:)
        real(R_)                    :: Max_Amount
        integer, allocatable:: Max_Amount_Index(:)
        integer                     :: i
    
        Max_Amount = MaxVal(Real(Group%Aver_Mark, R_))
        Max_Amount_Index = Pack([(i, i = 1, Size(Group))],[Group%Aver_mark==Max_Amount])
