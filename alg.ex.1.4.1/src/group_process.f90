...
contains
   pure recursive subroutine Max_and_sum (Group, i, Boys_Max_Value, Girls_Max_Value, &
           Boys_Amount, Girls_Amount, Number_of_Boys, Number_of_Girls)
     type(student), intent(inout) :: Group(:)
     integer, intent(in) :: i
     real(R_), intent(inout) :: Boys_Max_Value, Girls_Max_Value, Boys_Amount, Girls_Amount
     integer, intent(inout) :: Number_of_Boys, Number_of_Girls

     if (i <= size(Group)) then
       if (Group(i)%Sex == MALE) then
         Boys_Amount = Boys_Amount + Group(i)%Aver_mark
       ...
       if (Group(i)%Sex == FEMALE) then
         Girls_Amount = Girls_Amount + Group(i)%Aver_mark
         Number_of_Girls = Number_of_Girls + 1         
         if (Group(i)%Aver_mark > Girls_Max_Value) &
           Girls_Max_Value = Group(i)%Aver_mark
       end if
       call Max_and_sum (Group, i+1, Boys_Max_Value, Girls_Max_Value, Boys_Amount, Girls_Amount, Number_of_Boys, Number_of_Girls)
     end if 
